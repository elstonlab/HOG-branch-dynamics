# -*- coding: utf-8 -*-
"""
Created on Tue Apr  7 11:24:14 2020
...
def scorefxn_helper(individual):
    ...
    return scorefxn1(inits, individual),
...
# MODEL
#############################################################################
def signal(t, data_Pbs2):
    sig = 0
    if t > 0:
        sig = np.interp(t, Pbs2_time, data_Pbs2)
    return sig

def model(initials, t, total_protein, params, data_Pbs2):
    Hog1AC, Hog1AN, Hog1PN, Hog1IN, Hog1IC, Hog1PC, Hog1KN = initials
    kAf, kAr, kAcat, kB, k1, k2f, k2r, k2cat, k3f, k3r, k3cat, k4, k5, k6f, k6r, k6cat = params #18
    Pbs2_tot, Hog1_tot = total_protein

    sig = signal(t, data_Pbs2)
    conv_sig = sig/100 * Pbs2_tot
    Hog1KC = Hog1_tot - Hog1AC - Hog1AN - Hog1PN - Hog1IN - Hog1IC - Hog1PC - Hog1KN
    ...
    dHog1AC = kAcat * Hog1KC + k6r * Hog1PC - k6f * Hog1AC - kB * Hog1AC + k1 * Hog1AN
    dHog1AN = kB * Hog1AC - k1 * Hog1AN - k2f * Hog1AN + k2r * Hog1PN + k3cat * Hog1KN
    dHog1PN = k2f * Hog1AN - k2r * Hog1PN - k2cat * Hog1PN
    dHog1IN = k2cat * Hog1PN - k3f * (conv_sig - Hog1KC - Hog1KN) * Hog1IN + k3r * Hog1KN - k4 * Hog1IN + k5 * Hog1IC
    dHog1IC = k4 * Hog1IN - k5 * Hog1IC + k6cat * Hog1PC - kAf * (conv_sig - Hog1KC - Hog1KN) * Hog1IC + kAr * Hog1KC
    dHog1PC = k6f * Hog1AC - k6r * Hog1PC - k6cat * Hog1PC
    dHog1KN = k3f * (conv_sig - Hog1KC - Hog1KN) * Hog1IN  - k3r * Hog1KN - k3cat * Hog1KN 

    return dHog1AC, dHog1AN, dHog1PN, dHog1IN, dHog1IC, dHog1PC, dHog1KN

def run_ss(inits, total_protein, learned_params):
    ss = fsolve(model, inits, args=(0, total_protein, learned_params, 0))
    return ss

def simulate_experiment(inits, total_protein, learned_params, time, data_Pbs2):
    odes = odeint(model, inits, time, args=(total_protein, learned_params, data_Pbs2))
    return odes
...
def scorefxn1(inits, learned_params):
    ...
    ss_inits = run_ss(inits, total_protein, params)
    if (ss_inits < 0).any():
        return ((63+69+18*2+27)*100)**2

    Hog1_ss = np.sum(ss_inits[:])
    wt_ss_check = np.concatenate([ss_inits[:], [Hog1_ss]], axis=0)
    check = 100 - wt_ss_check
    if (check < 0).any():
        return ((63+69+18*2+27)*100)**2
    ...
    # WILDTYPE
    for dose, data_nuc, data_phospho, data_Pbs2 in zip(hog1_doses, wt_nuc_data, wt_phospho_data, wt_Pbs2_data):
        odes = simulate_experiment(ss_inits, total_protein, params, time, data_Pbs2)
        Hog1A = (odes[:,0] + odes[:,1])/total_protein[1]*100
        error_phospho = np.sum((data_phospho - Hog1A[closest_idxs_phospho])**2)
        mse_total += error_phospho
        Hog1N = (odes[:,1] + odes[:,2])/total_protein[1]*100
...
if __name__ == '__main__':
    ...
    #base filename
    save_filename = '200506_Model_1_2_3_4_5_6_maPbs2.txt'

    # Paths to longleaf data
    base_folder = '/nas/longleaf/home/rvdv/Prescaled_input_Pbs2/'
    wt_phospho_folder = base_folder + 'WT_phospho'
    wt_nuc_folder = base_folder + '90percent/WT_nuc'
    wt_Pbs2_folder = base_folder + 'WT_Pbs2'

    # Paths to local data
#    base_folder = 'C:/Users/Rozemarijn/Documents/Universiteit/Masterstage2/Research/Modelling/Inputs/Prescaled_input_Pbs2/'
#    wt_phospho_folder = base_folder + 'WT_phospho'
#    wt_nuc_folder = base_folder + '90percent/WT_nuc'
#    wt_Pbs2_folder = base_folder + 'WT_Pbs2'
    ...
    Pbs2_time, wt_Pbs2_data = load_csv_data(wt_Pbs2_folder)

    # Protein concentrations
    Pbs2_tot = molarity_conversion(4076)
    Hog1_tot = molarity_conversion(8225)
    total_protein = [Pbs2_tot, Hog1_tot] #mM

    # initial values
    Hog1AC = 0
    Hog1AN = 0
    Hog1PN = 0
    Hog1IN = 0.23 * Hog1_tot
    Hog1IC = 0.77 * Hog1_tot
    Hog1PC = 0
    Hog1KN = 0
    inits = [Hog1AC, Hog1AN, Hog1PN, Hog1IN, Hog1IC, Hog1PC, Hog1KN]
