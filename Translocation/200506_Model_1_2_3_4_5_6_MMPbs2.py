...
def model(initials, t, Hog1_tot, params, data_Pbs2):
    Hog1AC, Hog1AN, Hog1IN = initials
    kA, KA, kb, k1, k2, K2, k3, K3, k4, k5, k6, K6 = params #18

    sig = signal(t, data_Pbs2)
    Hog1IC = Hog1_tot - Hog1AC - Hog1AN - Hog1IN
    ...
    dHog1AC = kA * sig * (Hog1IC) / (KA + Hog1IC) - kb * Hog1AC + k1 * Hog1AN - k6 * Hog1AC / (K6 + Hog1AC)
    dHog1AN = kb * Hog1AC - k1 * Hog1AN - k2 * Hog1AN / (K2 + Hog1AN) + k3 * sig * Hog1IN / (K3 + Hog1IN)
    dHog1IN = k2 * Hog1AN / (K2 + Hog1AN) - k3 * sig * Hog1IN / (K3 + Hog1IN) - k4 * Hog1IN + k5 * Hog1IC

    return dHog1AC, dHog1AN, dHog1IN

def run_ss(inits, Hog1_tot, learned_params):
    ss = fsolve(model, inits, args=(0, Hog1_tot, learned_params, 0))
    return ss

def simulate_experiment(inits, Hog1_tot, learned_params, time, data_Pbs2):
    odes = odeint(model, inits, time, args=(Hog1_tot, learned_params, data_Pbs2))
    return odes
...
def scorefxn1(inits, learned_params):
    ...
    ss_inits = run_ss(inits, Hog1_tot, params)
    if (ss_inits < 0).any():
    ...
    for dose, data_nuc, data_phospho, data_Pbs2 in zip(hog1_doses, wt_nuc_data, wt_phospho_data, wt_Pbs2_data):
        odes = simulate_experiment(ss_inits, Hog1_tot, params, time, data_Pbs2)
        Hog1A = odes[:,0] + odes[:,1]
...
if __name__ == '__main__':
    ...
    #base filename
    save_filename = '200506_Model_1_2_3_4_5_6_MMPbs2.txt'
