...
# MODEL
#############################################################################
def signal(t, data_phospho):
    sig = 0
    if t > 0:
        sig = np.interp(t, phospho_time, data_phospho)
    return sig

def model(initials, t, Hog1_tot, params, data_phospho):
    Hog1AC, Hog1PN, Hog1IN, Hog1IC, Hog1PC, Hog1KN = initials
    kAf, kAr, kAcat, kB, k1, k2f, k2r, k2cat, k3f, k3r, k3cat, k4, k5, k6f, k6r, k6cat = params #18

    Hog1A = signal(t, data_phospho)     
    Hog1AN = Hog1A - Hog1AC - Hog1PC - Hog1PN
    Hog1KC = Hog1_tot - Hog1A - Hog1KN - Hog1IN - Hog1IC
    ...
    dHog1IN = k2cat * Hog1PN - k3f * Hog1IN + k3r * Hog1KN - k4 * Hog1IN + k5 * Hog1IC
    dHog1IC = k4 * Hog1IN - k5 * Hog1IC + k6cat * Hog1PC - kAf * Hog1IC + kAr * Hog1KC
    dHog1PC = k6f * Hog1AC - k6r * Hog1PC - k6cat * Hog1PC
    dHog1KN = k3f * Hog1IN  - k3r * Hog1KN - k3cat * Hog1KN 

    return dHog1AC, dHog1PN, dHog1IN, dHog1IC, dHog1PC, dHog1KN

def run_ss(inits, Hog1_tot, learned_params):
    ss = fsolve(model, inits, args=(0, Hog1_tot, learned_params, 0))
    return ss

def simulate_experiment(inits, Hog1_tot, learned_params, time, data_phospho):
    odes = odeint(model, inits, time, args=(Hog1_tot, learned_params, data_phospho))
    return odes
...
def scorefxn1(inits, learned_params):
    ...
    # WILDTYPE
    for dose, data_nuc, data_phospho in zip(hog1_doses, wt_nuc_data, wt_phospho_data):
        odes = simulate_experiment(ss_inits, Hog1_tot, params, time, data_phospho)
        Hog1A = np.interp(time, phospho_time, data_phospho)
        Hog1AN = Hog1A - odes[:,0] - odes[:,4] - odes[:,1]
        Hog1N = Hog1AN + odes[:,1] + odes[:,2] + odes[:,5]
...
if __name__ == '__main__':
    ...
    #base filename
    save_filename = '200504_Model_1_2_3_4_5_6_ma.txt'
...
#    wt_nuc_folder = base_folder + '90percent/WT_nuc'
    ...
    Hog1PC = 0
    Hog1KN = 0
    inits = [Hog1AC, Hog1PN, Hog1IN, Hog1IC, Hog1PC, Hog1KN]
