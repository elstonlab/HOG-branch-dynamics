...
def scorefxn1(inits, total_protein, learned_params):
    ...
    Hog1_ss = np.sum(wt_ss_inits[1:4])
    wt_ss_check = np.concatenate([wt_ss_inits[1:4], [Hog1_ss]], axis=0)
    ...
    for dose, data_phospho, data_nuc in zip(hog1_doses, wt_phospho_data, wt_nuc_data):
        ...
        if check_odes(odes):
            return ((9*5)*100)**2
        Hog1_phospho = (odes[:,1]+odes[:,2]+odes[:,3])/total_protein[1]*100 #calc as a percentage
        ...
        Hog1_nuc = (odes[:,3]+odes[:,4])/total_protein[1]*100 # percentage
        before = np.sum((data_nuc[0]-Hog1_nuc[closest_idxs_nuc[0]])**2)
        if dose==150:
            heavy = np.sum((data_nuc[1:3]-Hog1_nuc[closest_idxs_nuc[1:3]])**2)
            after = np.sum((data_nuc[3:]-Hog1_nuc[closest_idxs_nuc[3:]])**2)
        elif dose==350:
            heavy = np.sum((data_nuc[1:5]-Hog1_nuc[closest_idxs_nuc[1:5]])**2)
            after = np.sum((data_nuc[5:]-Hog1_nuc[closest_idxs_nuc[5:]])**2)
        else:
            heavy = np.sum((data_nuc[1:7]-Hog1_nuc[closest_idxs_nuc[1:7]])**2)
            after = np.sum((data_nuc[7:]-Hog1_nuc[closest_idxs_nuc[7:]])**2)
        error_nuc = before + 2*heavy + after #calc sum of squares since between 0 and 1
        mse_total += error_nuc
    return mse_total
...
if __name__ == '__main__':
    ...
    #base filename
    save_filename = '200615_OptimizedModel_Hog1ACcomp_v1.txt'
...
#    wt_nuc_folder = base_folder + '60percent/WT_nuc'
    ...
    C_tot = 0.2 * Hog1_tot
    total_protein = [Sho1_tot, Hog1_tot, C_tot, 0] #mM
    ...
    Glycerol = 0.0001
    inits = [Sho1, Hog1_AC, Hog1_AC2, Hog1_AN, Hog1_IN, Glycerol]
