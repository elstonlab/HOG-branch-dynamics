...
phospho_palette = {150:'#92C5DE', 350: '#4393C3', 550:'#2166AC'}
#phospho_palette = {150:'#B9BAB9', 350: '#7C7C7C', 550:'#000100'}
#phospho_palette = {150:'#F5A582', 350: '#D6604D', 550:'#B21A2C'}
#phospho_palette = {150:'#C2A5CF', 350: '#9970AB', 550:'#762A83'}
nuc_palette = {150:'#C2A5CF', 350: '#9970AB', 550:'#762A83'}
exp_palette = {150:'#B9BAB9', 350: '#7C7C7C', 550:'#000100'}
#exp_palette = {150:'#F5A582', 350: '#D6604D', 550:'#B21A2C'}
...
def fit_data_to(model_fxns, top_params, params_constants, initials, time, param, sig):
        sims = []
        for idx, params in enumerate(top_params):
            ...
            for i in range(1,601):
                areas2[i-1]=(data[i-1,2]+data[i,2])/2*10/params_constants[1]
...
def plt_param_integrals(model_fxns, top_params, params_constants, initials,  doses, time, param,
                        ci =95,
                        save_fig=''):
   ...
   # else:
    ...
    #print(exp_time)
    for sig in doses:
    #ax1.plot(exp_time, exp_data[0], color=palette.get(150), label = str(int(150)))
    #ax1.plot(exp_time, exp_data[1], color=palette.get(350), label = str(int(350)))
    #ax1.plot(exp_time, exp_data[2], color=palette.get(550), label = str(int(550)))
        sims = fit_data_to(model_fxns, top_params, params_constants, initials, time, param, sig)
        ax1 = sns.tsplot(sims, time[0:600],  ci = ci, dashes = dashes, color=palette.get(sig), condition=str(int(sig)), legend=True)
...
#         ax1.set_xlim(-2,62)
   ...
   # ax1.grid(color='grey', linestyle='-', axis='y', linewidth=.5)
    ...
    ax1.legend(loc='upper left', fontsize='medium',frameon=True, title='KCl (mM)',fancybox=True)
    plt.setp(ax1.get_legend().get_title(), fontsize='17') # for legend title
    ax1.set_xlabel('Time (min)', fontweight= 'bold', fontsize='17')
   # if param == 'phospho':
    ax1.set_ylabel('Integral of nuclear\nHog1$_p$$_p$ (a.u.)', fontweight= 'bold', fontsize='17')
   # else:
   #     ax1.set_ylabel('Nuclear Hog1 (%)', fontweight= 'bold',fontsize='22')
...
#                 else:
               ...
               # Hog1_IC = (params_constants[2] - data[:,1] - data[:,2] - data[:,3])/params_constants[1]*100
               # Hog1_AC = data[:,1]/params_constants[1]*100
             #   Hog1_AC2 = data[:,3]/params_constants[2]*100
               # Hog1_AN = data[:,2]/params_constants[1]*100
               # Hog1_IN = data[:,3]/params_constants[1]*100
               # sln1 = data[:,0]/params_constants[0]*100
              #  sho1 = data[:,1]/params_constants[1]*100
              #  comp = data[:,3]/params_constants[2]*100
                if param == 'phospho':
                    ax1.plot(time, Hog1_AC, color=palette.get(sig))
                    ax2.plot(time, Hog1_AN, color=palette.get(sig))
                    ax3.plot(time, Hog1_IN, color=palette.get(sig))
                    ax4.plot(time, Hog1_IC, color=palette.get(sig))
                    ax1.set_ylim([0,100])
                elif param == 'nuc':
...
def fit_data_to_list(model_fxns, top_params, params_constants, initials, time, param, dose,
                        ss = False):
        sims = []
        for idx, params in enumerate(top_params):
           # params[20] = 0
...
def plt_param_cis(model_fxns, top_params, params_constants, initials,  doses, time, param,
                        exp_data=None, exp_time=None, ss=False, ci= 95,
                        save_fig=''):
    plt.clf()
    font = {'family' : 'Calibri',
        'weight' : 'bold',
   ...
   # else:
    ...
    diff = []
    for sig in doses:
        sims = fit_data_to_list(model_fxns, top_params, params_constants, initials, time, param, sig, ss=ss)
        #print(time)
        ax1 = sns.tsplot(sims, time,  ci = ci, dashes = dashes, color=palette.get(sig), condition=str(int(sig)), legend=False)
#         abba=np.empty([1,10])
#         for i in range(10):
#                 abba[0,i-1]=sims[i-1][31]
#         beta=np.mean(abba)
#         diff.append(beta)
#     diff2=diff[2]-diff[1]
#     print(diff2)
    ...
    if exp_data:
        ...
        for sig, data in zip(doses, exp_data):
                if sig == 0:
                    ...
                    mark = 'D'
                ax1.plot(exp_time, data, marker=mark, markersize=9, linestyle="-", linewidth=2, color=palette.get(sig), fillstyle='full', label = str(int(sig)))#, fillstyle=fill, linestyle="None"), palette.get(sig), mec='black' (outside edge)
                if param == 'nuc':
                    ax1.set_ylim([15,65])
                    ax1.set_xlim([0, 60])
                else:
...
#         ax1.set_xlim(-2,62)
    ...
    ax1.spines['right'].set_visible(False)
    ax1.spines['top'].set_visible(False)
    ax1.legend(loc='upper right', fontsize='medium',frameon=False, framealpha=0.5, title='KCl (mM)')
    plt.setp(ax1.get_legend().get_title(), fontsize='17') # for legend title
    ax1.set_xlabel('Time (min)', fontweight= 'bold', fontsize='17')
    from matplotlib.ticker import (MultipleLocator, FormatStrFormatter,
                               AutoMinorLocator)
    if param == 'phospho':
        ax1.set_ylabel('Hog1$_p$$_p$ (%)', fontweight= 'bold', fontsize='17')
        ax1.yaxis.set_major_locator(MultipleLocator(25))
    else:
        ax1.set_ylabel('Nuclear Hog1 (%)', fontweight= 'bold',fontsize='17')
