...
# 191004 -> M2 + a -> fit to just Hog1 data
# 191010 -> fit to Hog1 and Pbs2 data
# 191205 -> fixed bug in positive feedback
# 200205 -> fit to my data, just 150 and 550 for Hog1
#        -> updated timepoint finding
#        -> updated 'check' to not limit osmo concentration b/c it was arbitrary
#        -> changed mutation rate from .2 to .1 (what it was orginally)
# 200301 -> updated protein concentrations to Ho 2018 Cell Sys measurements
#        -> fixed bug in scorefxn (t110a 0)
# 200307 -> fixed data loading bug that hadn't been an issue until using dogwood
# 200504 -> fitting to translocation data, and 350mM phosphodata
...
MAPK_n = 0.00540042381*MAPK_t*.2
gly = 0.00001 # placeholder (so not 0)
PTP = molarity_conversion(443+1324) # ptp2+ptp3

# doses
hog1_doses = [150000, 350000, 550000]
hog1_t100a_doses = [0, 150000, 550000]
...
def scorefxn1(inits, total_protein, learned_params):
    ...
    for dose, data_phospho, data_nuc in zip(hog1_doses, mapk_wt_data, wt_nuc_data):
        ...
        if dose == 150000:
            ...
            error_active = ((map2k_wt_data[0] - map2k[closest_idxs_other])**2).mean()
            mse_total += error_active
        elif dose == 550000:
            map2k = odes[:,1]/total_protein[1]*100
            error_active = ((map2k_wt_data[1] - map2k[closest_idxs_other])**2).mean()
            mse_total += error_active
    ...
    for dose, data in zip(hog1_t100a_doses, mapk_t100a_data):
        if dose == 0:
            ...
            if dose == 150000:
                map2k = odes[:,1]/total_protein[1]*100
                error_active = ((map2k_t100a_data[0] - map2k[closest_idxs_other])**2).mean()
                mse_total += error_active
            elif dose == 550000:
                map2k = odes[:,1]/total_protein[1]*100
                error_active = ((map2k_t100a_data[1] - map2k[closest_idxs_other])**2).mean()
                mse_total += error_active
