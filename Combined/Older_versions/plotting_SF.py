...
#                 else:
                data = simulate_wt_experiment(model_fxns.m, initials, params_constants, sig, params[:-1], time)
...
def fit_data_to_list(model_fxns, top_params, params_constants, initials, time, param, dose, ptpD=False,
        ...
        for idx, params in enumerate(top_params): 
            if ss:
                ss_data = run_ss(model_fxns.m, initials, params_constants, params)
                data = simulate_wt_experiment(model_fxns.m, ss_data, params_constants, dose, params[:-1], time)
            else:
                data = simulate_wt_experiment(model_fxns.m, initials, params_constants, dose, params[:-1], time)
            if param == 'phospho':
