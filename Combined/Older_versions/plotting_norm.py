...
#                 else:
                ...
                phospho = (data[:,2]+data[:,3])/params_constants[2]
                nuc = (data[:,3]+data[:,4])/params_constants[2]
                if param == 'phospho':
...
def fit_data_to_list(model_fxns, top_params, params_constants, initials, time, param, dose, ptpD=False,
        ...
        for idx, params in enumerate(top_params): 
            ...
            if param == 'phospho':
                species = (data[:,2]+data[:,3])/params_constants[2]
            elif param == 'nuc':
                species = (data[:,3]+data[:,4])/params_constants[2]
            else:
                print('no data')
            sims.append(species)
