...
def norm_nuc_data(data):
    max_data = max(data[-1]) # norm experimental data to 550 max
    print(max_data)
    data /= max_data
    return data
...
def scorefxn1(inits, total_protein, learned_params):
    ...
    for dose, data_phospho, data_nuc in zip(reversed(hog1_doses), reversed(wt_phospho_data), reversed(wt_nuc_data)):
        ...
        if dose == 550:
            idx_max = np.argmax(Hog1_nuc) #find the index of the max nuclear value
            nuc_norm = Hog1_nuc[idx_max] #normalizing factor
        Hog1_nuc /= nuc_norm
...
if __name__ == '__main__':
    ...
    #base filename
    save_filename = '200422_Model28_MM_max_norm.txt'

    # Paths to longleaf data
    base_folder = '/nas/longleaf/home/rvdv/Normalized_Input_max_mean/'
