...
# 200422 -> change normalizing nuc data from max to steady state
#        -> normalizing to averaged data (now takes raw data again)
...
#############################################################################
# Norm nuc experimental data
#############################################################################
def norm_nuc_data(data):
    ss_data = data[-1][0] # norm experimental data to 550 baseline 
    data /= ss_data
    return data
...
def scorefxn1(inits, total_protein, learned_params):
    ...
    # WILDTYPE
    for dose, data_phospho, data_nuc in zip(reversed(hog1_doses), reversed(wt_phospho_data), reversed(wt_nuc_data)):
        # print(dose)
        # print(data_phospho)
        # print(data_nuc)

        odes = simulate_wt_experiment(wt_ss_inits, total_protein, dose, params, time)
        Hog1_phospho = (odes[:,2]+odes[:,3])/total_protein[2] #still normalize phospho-Hog1 to total protein
        error_phospho = np.sum((data_phospho - Hog1_phospho[closest_idxs_phospho])**2) #sum of squares since between 0 and 1
        mse_total += error_phospho
        Hog1_nuc = odes[:,3]+odes[:,4] #add nuc species together
        if dose == 550:
            nuc_norm = Hog1_nuc[0]
        Hog1_nuc /= nuc_norm # normalize to t = 0 (should be steady state)
        error_nuc = np.sum((data_nuc- Hog1_nuc[closest_idxs_nuc])**2) #calc sum of squares not in between 0 and 1
        mse_total += error_nuc
...
if __name__ == '__main__':
    ...
    #base filename
    save_filename = '200422_Model28_MM_ss_norm.txt'

    # Paths to longleaf data
    base_folder = '/nas/longleaf/home/rvdv/Normalized_Input_ss_mean/'
    # base_folder = 'C:/Users/sksuzuki/Downloads/Rozemarijn/Normalized_Input/'
...
#    wt_nuc_data = norm_nuc_data(wt_nuc_data)
