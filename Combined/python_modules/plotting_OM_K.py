...
def plt_param_behaviors(model_fxns, top_params, plt_top, params_constants, initials,  doses, time, param,
    ...
    for sig in doses:
        if param == 'all Hog1':
            ...
            ax2.plot(time, (params_constants[1]-data[:,1]-data[:,2]-data[:,3])/params_constants[1]*100, label = labels[0], color = '#CCBB44', linewidth=2)
            ax1.plot(time, data[:,1]/params_constants[1]*100, label = labels[1], color = '#228833', linewidth=2)
            ax1.plot(time, data[:,2]/params_constants[1]*100,  label = labels[2], color = '#4477AA', linewidth=2)
            ax2.plot(time, data[:,3]/params_constants[1]*100,  label = labels[3], color = '#66CCEE', linewidth=2)
