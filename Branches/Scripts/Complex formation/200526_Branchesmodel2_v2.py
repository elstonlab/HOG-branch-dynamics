...
def simulate_sln1_experiment(inits, total_protein, sig, learned_params, time):
    base_osmo, k1, K1, k2, K2, k3, K3, k4, K4, k5f, k5r, k6f, k6r, k7, K7, k8, k9, k10, kcat = learned_params #16
    learned_params = base_osmo, k1, K1, 0, 0, k3, K3, 0, 0, k5f, k5r, 0, 0, k7, K7, k8, k9, k10, kcat
    #solve odes:
    odes = odeint(model, inits, time, args=(total_protein, sig, learned_params))
    return odes
...
def simulate_sho1_experiment(inits, total_protein, sig, learned_params, time):
    base_osmo, k1, K1, k2, K2, k3, K3, k4, K4, k5f, k5r, k6f, k6r, k7, K7, k8, k9, k10, kcat = learned_params #16
    learned_params = base_osmo, 0, 0, k2, K2, 0, 0, k4, K4, 0, 0, k6f, k6r, k7, K7, k8, k9, k10, kcat
    #solve odes:
    odes = odeint(model, inits, time, args=(total_protein, sig, learned_params))
    return odes
...
def scorefxn1(inits, total_protein, learned_params):
    ...
    Hog1_ss = np.sum(wt_ss_inits[2:3])
    Hog1_ss_check = np.concatenate([wt_ss_inits[2:3], [Hog1_ss]], axis=0)
    check = total_protein[1] - Hog1_ss_check
    if (check < 0).any():
        return ((63+69+18*2+27)*100)**2
    
    Pbs2_ss = np.sum(wt_ss_inits[0:2])
    Pbs2_ss_check = np.concatenate([wt_ss_inits[0:2], [Pbs2_ss]], axis=0)
    check = total_protein[0] - Pbs2_ss_check
    if (check < 0).any():
        return ((63+69+18*2+27)*100)**2
    ...
    for dose, data_wt in zip(hog1_doses, wt_data):
        ...
        Hog1_wt = odes[:,3]/total_protein[1]*100
    ...
    for dose, data_sln1 in zip(hog1_doses, sln1_data):
        ...
        Hog1_sln1 = odes[:,3]/total_protein[1]*100
    ...
    for dose, data_sho1 in zip(hog1_doses, sho1_data):
        ...
        Hog1_sho1 = odes[:,3]/total_protein[1]*100
...
if __name__ == '__main__':
    ...
    #base filename
    save_filename = '200526_Branchesmodel2_v2.txt'
...
#    sho1_folder = base_folder + 'Sho1_phospho'
    ...
    total_protein = [Pbs2_tot, Hog1_tot, 0] #mM
    ...
    Hog1A = 0
    Glycerol = 0.0001
    inits = [Sln1Pbs2A, Sho1Pbs2A, Hog1IPbs2A, Hog1A, Glycerol]
